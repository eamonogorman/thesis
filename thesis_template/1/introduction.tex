...
\section{Motivation for Researching Cool Evolved Stellar Winds}\label{sec:1}
...
\section{On the Nature of Cool Evolved Stellar Winds}\label{sec:2}
...
\end{equation}
where each of these terms have units of cm\,s$^{-2}$. The term on to the left of Equation \ref{eq:moment} is the acceleration, which is produced by the gas pressure gradient (first term on right), the gravity (second term on right), and some other force(s) which are contained in $f$. The gas pressure gradient term is directed outwards (positive) because $dP/dr < 0$. 
...
\begin{align}
 u(r) &= \frac{v^2}{2} + \frac{5\mathcal{R}T}{2\mu}-\frac{GM_{\star}}{r} \nonumber \\
 & {} =u(r_{0}) + W(r) + Q(r)
\end{align}
which states that the total energy per unit mass is equal to the initial energy per unit mass, $u(r_{0})$, at the lower boundary $r_{0}$, plus the energy added to the wind in the form of the work done by the force, $W(r)$, and the heat deposition, $Q(r)$. in other words, the total energy added to the wind per unit mass is used to increase the kinetic energy and the enthalpy of the wind, and to lift it out of the gravitational potential well. We can also compare the energy of the wind at the photosphere and at infinity.


\section{Stellar Wind Driving Mechanisms Across the HR Diagram}\label{sec:1.4}
Equation \ref{eq:moment} was the cornerstone of many early attempts to explain the dynamics of the solar corona. \cite{chapman_1957} assumed a static corona (i.e., the acceleration term was zero) and that the pressure gradient was the only outward force (i.e., $f=0$). They calculated the temperature gradient from the conservation of conductive flux. Their results were unphysical, however, as they found that the density goes to infinity at large distances, and that the pressure was many orders of magnitude greater than that of the ISM. \cite{parker_1958} correctly assumed that there was a continuous isothermal outflow of material from the Sun caused only by the pressure gradient term in Equation \ref{eq:moment} (i.e., $f=0$). He used the mass continuity equation, and the equation of state, to replace the pressure gradient term with a function depending only on velocity. Parker coined the solution to his simple analytical model as the ``solar wind'' and the preditions his model made for the solar velocity were confirmed shortly afterwards by some of the first space probes \citep[e.g.,][]{neugebauer_1962}. The assumptions of a radially expanding and isothermal outflow are not fully accurate in reality, however, and Parker's solution is an approximate characterization of the observed solar wind.
...
possible methods for cool stars bennet
Basel heating
